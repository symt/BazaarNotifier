...
public class ProfitCalculator {
    ...
    private static double moneyOnStartup = moneyStoredInSellOffers() + moneyStoredInBuyOrders() + getPurse();
    public static double moneyNotFromBazaar = 0;
    public static double moneyOnBazaarLeave = 0;


    public static double calculateProfit(){
        return getPurse() + moneyStoredInBuyOrders() + moneyStoredInSellOffers() + bank - moneyOnStartup;
    }
    ...
    private static double getPurseFromSidebar() {
        ...
        for (Score score : scores) {
            ...
            if (ScorePlayerTeam.formatPlayerName(team, score.getPlayerName()).contains("Purse")) {
                if(ScorePlayerTeam.formatPlayerName(team, score.getPlayerName()).contains(")")){// coins get added to your purse
                    String purse = StringUtils.stripControlCodes(ScorePlayerTeam.formatPlayerName(team, score.getPlayerName()));
                    int i = purse.indexOf("(");
                    String s = purse.substring(i + 1, purse.length()-1);
                    purse =  purse.replace(s, "").replaceAll("[^0-9 .]", "");
                    return Float.parseFloat(purse);
                }else {
                    String purse = StringUtils.stripControlCodes(ScorePlayerTeam.formatPlayerName(team, score.getPlayerName())).replaceAll("[^0-9 +.]", "");
                    return Float.parseFloat(purse);
                }
            }
        }
        ...
        if (chest != null) {
            if (chest.getStackInSlot(11) != null) {
                if (chest.getStackInSlot(11).getDisplayName().toLowerCase().contains("deposit coins")) {
                    if (bank == 0) {
                        bank = Double.parseDouble(StringUtils.stripControlCodes(chest.getStackInSlot(11).getTagCompound().getCompoundTag("display").getTagList("Lore", 8)
